...
namespace obd2NET {
    ...
    public static class Vehicle {
        ...
        public static uint Speed(IOBDConnection obdConnection) {
            ...
            return (response.Value.Length >= 2) ? Convert.ToUInt32(response.Value.ElementAt(1)) : 0;
        }
        ...
        public static int EngineTemperature(IOBDConnection obdConnection) {
            ...
            return (response.Value.Length >= 2) ? Convert.ToInt32(response.Value.ElementAt(1)) - 40 : 0;
        }
        ...
        public static uint RPM(IOBDConnection obdConnection) {
            ...
            uint rpm1 = Convert.ToUInt32(response.Value.ElementAt(1));
            uint rpm2 = Convert.ToUInt32(response.Value.ElementAt(2));
        ...
        public static uint ThrottlePosition(IOBDConnection obdConnection) {
            ...
            return (response.Value.Length >= 2) ? (Convert.ToUInt32(response.Value.ElementAt(1)) * 100) / 255 : 0;
        }
        ...
        public static uint CalculatedEngineLoadValue(IOBDConnection obdConnection) {
            ...
            if (response.HasValidData())
                throw new QueryException(
                    "Calculated engine load value couldn't be queried, the controller returned no data");
        ...
        public static uint FuelPressure(IOBDConnection obdConnection) {
            ...
            return (response.Value.Length >= 2) ? Convert.ToUInt32(response.Value.ElementAt(1)) * 3 : 0;
        }
        ...
        public static bool MalfunctionIndicatorLamp(IOBDConnection obdConnection) {
            ...
            if (response.HasValidData())
                throw new QueryException(
                    "Malfunction indicator lamp couldn't be queried, the controller returned no data");
            if (response.Value.Length == 0) return false;

            return Convert.ToBoolean((response.Value.ElementAt(1) >> 7) & 1);
        }
        ...
        public static List<DiagnosticTroubleCode> DiagnosticTroubleCodes(IOBDConnection obdConnection) {
            ...
            if (response.HasValidData())
                throw new QueryException(
                    "Diagnostic trouble codes couldn't be queried, the controller returned no data");
            if (response.Value.Length < 2)
                throw new QueryException(
                    "Diagnostic trouble codes couldn't be queried, received data was not complete");
            ...
            for (int i = 0; i < response.Value.Length; i += 2) // each error code got a size of 2 bytes
            {
                ...
                if (!troubleCode.All(b => b == default(Byte))
                ) // if the byte array is not entirely empty, add the error code to the parsed list
